(ns acme.frontend.eval-region
  (:require [acme.frontend.node :as n]
            ...
            ["@codemirror/state" :as state :refer [StateEffect StateField Transaction]]
...
(defn node-at-cursor
  ...
  ([^js state from]
   (some->> (n/nearest-touching state from -1)
            (#(when (or (n/terminal-type? (n/node-type %))
                        (<= (n/start %) from)
                        (<= (n/end %) from))
                (cond-> %
                  (or (n/top? %)
                      (and (not (n/terminal-type? (n/node-type %)))
                           (< (n/start %) from (n/end %))))
                           ...
                           (j/let [^:js {:keys [from to]} (current-range state)]
                             (when (not= from to)
                               (dispatch (j/lit {:changes {:from from :to to :insert ""}
                                                 :annotations (user-event-annotation "delete")})))
                             true))]
    #js[modifier-field
        (.of keymap
...
(defn cursor-node-string [^js state]
  (n/guard (some->> (node-at-cursor state)
                    (range-str state))
           (complement str/blank?)))

(defn top-level-string [^js state]
  (n/guard (some->> (top-level-node state)
                    (range-str state))
           (complement str/blank?)))
